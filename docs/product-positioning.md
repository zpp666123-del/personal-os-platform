...
| 数据图表 | 已用数字卡片展示访问数 | 需要趋势/渠道分析时，再评估 Chart.js |
...
1. 作品资产模块增强。精选项目已支持角色、结果、封面和链接；头像、作品封面、PDF 简历已支持 CloudBase Storage 上传。
2. Inbox 通知。状态流转、备注与 CSV 导出已先落地。
3. 模板和主题。`signal-light`、`build-story`、`craft-dark` 已先落地公开页样式差异。
4. 访问统计。公开页访问计数已先落地；趋势图后续再加。
5. 自定义域名和商业化。
